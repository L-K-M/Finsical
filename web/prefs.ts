...
// and sound. The tank page owns persistence and rendering: this page
// renders the state it pushes back (op:"state" carries `crt`,
// `machine` and `sound` snapshots) and posts intents: crtEnabled,
// crtConfig, machine, soundConfig.
...
let described: SliderSpec | SoundItem | null = null;
function describe(spec: SliderSpec | SoundItem | null): void {
  described = spec;
  descEl.textContent = "";
  if (pane === "machine") {
  ...
  const value = "key" in spec ? (spec.fmt ?? pct)(cfg[spec.key])
    : spec.value?.();
  descEl.append(el("span", "osm-label", value === undefined
                     ? spec.label : `${spec.label}: ${value}`),
                ` — ${spec.blurb}`);
}
