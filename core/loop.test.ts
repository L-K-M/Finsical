...
import { drawOrFallback, MAX_FRAME_MS, planFrame } from "./loop.js";
...
describe("drawOrFallback", () => {
  ...
  it("draws healthy keys and never calls the fallback", () => {
    const broken = new WeakSet<object>();
    const draw = vi.fn(), fallback = vi.fn();
    drawOrFallback(broken, {}, draw, fallback);
    expect(draw).toHaveBeenCalledOnce();
    expect(fallback).not.toHaveBeenCalled();
  });

  it("falls back for good after one throw and logs it once", () => {
    const warn = vi.spyOn(console, "warn").mockImplementation(() => {});
    const broken = new WeakSet<object>();
    const bad = {}, good = {};
    const badDraw = vi.fn(() => { throw new RangeError("0x0 frame"); });
    const goodDraw = vi.fn(), fallback = vi.fn();
    for (let i = 0; i < 3; i++) {
      drawOrFallback(broken, bad, badDraw, fallback);
      drawOrFallback(broken, good, goodDraw, fallback);
    }
    expect(badDraw).toHaveBeenCalledOnce();
    expect(fallback).toHaveBeenCalledTimes(3);
    expect(goodDraw).toHaveBeenCalledTimes(3);
    expect(broken.has(bad)).toBe(true);
