...
export async function inflateCap(data: Uint8Array, format: InflateFormat,
  ...
  return typeof DecompressionStream === "function"
    ? inflateNative(data, format, maxBytes)
    : inflateFflate(data, format, maxBytes);
}
...
export async function inflateNative(data: Uint8Array,
  ...
  try {
    for (;;) {
      ...
      if (total > maxBytes) {
        await reader.cancel();
        throw new Error(`inflate exceeded ${maxBytes} bytes`);
      }
      chunks.push(value);
    }
...
export async function inflateFflate(data: Uint8Array,
  ...
  if (format === "deflate") new Unzlib(ondata).push(data, true);
  else new Inflate(ondata).push(data, true);
  const out = new Uint8Array(total);
  let o = 0;
  for (const c of chunks) { out.set(c, o); o += c.byteLength; }
  return out;
