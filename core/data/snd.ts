...
// MACE 3:1 coefficient table — the same 128×4 big-endian u16s as
// tools/az/mace_tab.bin (FFmpeg mace table, LGPL), base64 so the table
// ships inside the JS bundle rather than as a fetchable asset.
const MACE_TAB_B64 =
...
const MACE_TAB2: number[][] = (() => {
  const bin = atob(MACE_TAB_B64);
  if (bin.length !== 1024)
    throw new Error(`mace table: expected 1024 bytes, got ${bin.length}`);
  const rows: number[][] = [];
  for (let i = 0; i < 128; i++) {
    const row: number[] = [];
    for (let j = 0; j < 4; j++)
      row.push((bin.charCodeAt(i * 8 + j * 2) << 8) |
               bin.charCodeAt(i * 8 + j * 2 + 1));
    rows.push(row);
  }
  return rows;
})();
...
export function mace3Decode(data: Uint8Array, npackets: number): Uint8Array {
  ...
  for (let j = 0; j < npackets; j++) {
    for (let k = 0; k < 2; k++) {
      ...
      for (const val of [pkt & 7, (pkt >> 3) & 3, pkt >> 5]) {
        const row = MACE_TAB2[(index & 0x7F0) >> 4]!;
        let cur = val < 4 ? row[val]! : -1 - row[7 - val]!;
        index += MACE_TAB1[val]! - (index >> 5);
        if (index < 0) index = 0;
